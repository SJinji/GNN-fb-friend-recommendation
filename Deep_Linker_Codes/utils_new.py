...
def load_project_data(break_portion):

    info = pd.read_csv("node_information.csv", delimiter = " ")

    #load graph
    idx_features_labels = np.array(info).astype(int)
    features = sp.csr_matrix(idx_features_labels[:, 1:-1], dtype=np.float32)
    idx = np.array(idx_features_labels[:, 0], dtype=np.int32)
    idx_map = {j: i for i, j in enumerate(idx)}

    edges_unordered = np.array(pd.read_csv("edges.csv", delimiter = ' ')).astype(np.int32)
    #edges_unordered = edges_unordered[edges_unordered[:,2]==1, 0:2]
    ...
    idx_train, idx_val = train_test_split(idx_train,test_size=0.1, random_state=1)   
